...
namespace Cuisine.Behaviors
{
    ...
    /// </summary>
    public class ContentDockBehavior : VisualParentBehavior<DocumentContainer>
    {
        /// <summary>
        /// Dock point
        /// </summary>
        ...
        protected override void OnAttached()
        {
            ...
            _parentStateChangeObserver.AddValueChanged(VisualParent);
            AssociatedObject.MouseEnter += OnMouseEnter;
            AssociatedObject.MouseUp += OnMouseUp;
            AssociatedObject.MouseLeave += OnMouseLeave;

            // This is important so content dock points can initialize their visibility
            OnParentStateChanged(VisualParent.State);
        }       
        ...
        private void OnMouseEnter(object sender, MouseEventArgs args)
        {
            ...
            switch (DockPoint)
            {
                case ContentDockPoint.Top:
                    IllustrateDocumentDock(Dock.Top, VisualParent.ActualHeight / 2, double.NaN);
                    break;
                case ContentDockPoint.Left:
                    IllustrateDocumentDock(Dock.Left, double.NaN, VisualParent.ActualWidth / 2);
                    break;
                case ContentDockPoint.Right:
                    IllustrateDocumentDock(Dock.Right, double.NaN, VisualParent.ActualWidth / 2);
                    break;
                case ContentDockPoint.Bottom:
                    IllustrateDocumentDock(Dock.Bottom, VisualParent.ActualHeight / 2, double.NaN);
                    break;
        ...
        {
            if (VisualParent.DockIllustrationPanel == null)
            {
                return;
            }
            ...
            VisualParent.DockIllustrationPanel.Children.Add(dockPaneIllustratingGrid);
        }
        ...
        {            
            ...
            VisualParent.Documents.Add(_tabItem);

            if (VisualParent.DocumentsTab != null)
            {
                VisualParent.DocumentsTab.SelectedItem = _tabItem;
            }
        }
        ...
        private void ClearIllustrations()
        {
            if (VisualParent.DockIllustrationPanel != null)
            {
                VisualParent.DockIllustrationPanel.Children.Clear();
            }

            VisualParent.Documents.Remove(_tabItem);            
        }
