...
namespace Cuisine.Windows
{
    ...
    public class DocumentContainer : ContentControl
    {
        ...
        public override void OnApplyTemplate()
        {
            ...
            if (DocumentsTab != null)
            {
                ...
                DocumentsTab.SelectionChanged -= DocumentsTab_SelectionChanged;
            }
        ...
        {
            Cuisine.Utilities.Validate.Assert<InvalidOperationException>(Content == null);

            Cuisine.Utilities.Validate.NotNull(childContainers, "childContainers");
            List<DocumentContainer> childContainerList = new List<DocumentContainer>(childContainers);
            Cuisine.Utilities.Validate.Assert<InvalidOperationException>((childContainerList.Count > 0) && (childContainerList.Count < 3));
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                if (i < childContainerList.Count)
                {
                    ...
                    if (isHorizontal)
                    {
                        Cuisine.Utilities.Validate.Assert<InvalidOperationException>((row == 0) && (position != column));
                        position = column;
                    }
                    else
                    {
                        Cuisine.Utilities.Validate.Assert<InvalidOperationException>((column == 0) && (position != row));
                        position = row;
                    }
                }
                else
        ...
        private void OnTabItemMouseUp(object sender, RoutedEventArgs args)
        {
            ...
            // can find the active document here but if user clicked on X to close document, close command happens next
            // see DocumentContainer.RemoveDocument()

            //RaiseActiveDocumentChanged();
            //((args.Source as System.Windows.Controls.TabControl).SelectedItem as Cuisine.Windows.DocumentContent).Content
        }
            ...
            switch (State)
            {
                ...
                case DocumentContainerState.SplitVertically:
                    ...
                    if (contentGrid != null)
                    {
                        ...
                        Cuisine.Utilities.Validate.NotNull(firstChild, "firstChild");

                        DocumentContainer secondChild = contentGrid.Children[2] as DocumentContainer;
                        Cuisine.Utilities.Validate.NotNull(secondChild, "secondChild");
    ...
    public delegate void DocumentClosingEventHandler(DocumentContent documentContent);

    public class ActiveDocumentChangedEventArgs
    {
        public DocumentContainer DocumentContainer { get ; private set;}
        public TabControl TabControl { get; private set; }

        public ActiveDocumentChangedEventArgs(DocumentContainer container, TabControl tabControl)
        {
